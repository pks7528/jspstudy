...
<body>
	<%--
		...
		4. 예시
			...
			<c:set valr="a" value="1000" scope="application"></c:set>
	--%>
	
	<c:set var="age" value="32" scope="page"></c:set>
	<c:set var="isAlive" value="${age <= 100}" scope="page"></c:set>
	bmi = 몸무게(kg) / (키(m) * 키(m))
	bmi < 20 : 저체중
	bmi < 25 : 정상
	나머지	 : 비만
	<c:set var="height" value="1.75" scope="page"></c:set>
	<c:set var="weight" value="70" scope="page"></c:set>
